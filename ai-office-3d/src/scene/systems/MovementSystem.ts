...
export class MovementSystem {
  update(entities: Map<string, AgentEntity>, dt: number): boolean {
    ...
    for (const entity of entities.values()) {
      ...
      if (dist < ARRIVE_THRESHOLD) {
        ...
        const hasMore = agent.walkPath != null && agent.walkPathIndex != null && agent.walkPathIndex < agent.walkPath.length - 1
        ...
        if (hasMore) {
          ...
          const ndx = next.x - tx
          const ndy = next.y - ty
          const vf = resolveWalkViewFacing(ndx, ndy)
          entity.apply({
            x: tx, y: ty,
            walkPathIndex: nextIndex,
            targetX: next.x, targetY: next.y,
            viewFacing: vf,
            facing: viewFacingToLR(vf),
          })
        ...
        const vf = atHome ? 'back' : resolveWalkViewFacing(dx, dy)
        entity.apply({
          x: tx, y: ty,
  ...
  static assignWalkPath(agent: Agent, points: { x: number; y: number }[]): Agent {
